		...
		if(data==null||data.size() == 0){
			...
			try {
				...
				for (int i = 0; i < data.size(); i++) {
					...
					db.execSQL("insert into friends (userId) values(?)",new Object[] { friendsEn.getUserId()});
					Cursor c = db.rawQuery("select * from userinfo where userId=? ", new String[]{friendsEn.getUserId()});
					if(c==null||c.getCount()==0){
						db.execSQL("insert into userinfo(userId, nickname ,portraitUrl,provinceId,districtId,cityId,sex,type) values(?,?,?,?,?,?,?,?)",
								new Object[] { friendsEn.getUserId(),friendsEn.getNickname(),friendsEn.getPortraitUrl(), 
								friendsEn.getProvinceId(), friendsEn.getDistrictId(), friendsEn.getCityId(), friendsEn.getSex(),1});
					}
					c.close();
				}
