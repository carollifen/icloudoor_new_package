...
public class AuthKeyActivity extends BaseActivity implements OnClickListener,
	...
	protected void onCreate(Bundle arg0) {
		...
		getNetworkData(this, "/user/api/getFamilyAddr.do", parm.toString(),
				true);
		getMyKey();
	}
		...
		MyRequestBody requestBody = new MyRequestBody(url, "{}",
				new Response.Listener<JSONObject>() {
					...
					public void onResponse(JSONObject response) {
						...
						UserCacheWrapper.setMedicalRecord(AuthKeyActivity.this,
								keyEn);
						if (keyEn != null) {
							List<KeyInfo> data = keyEn.getData();
							if (data == null || data.size() == 0) {
				...
				for (int j = 0; j < chekbleData.get(i).size(); j++) {
					if (chekbleData.get(i).get(j)) {
						...
						if (isCarDoor) {
							...
							map.put("zoneUserId", keyInfo.getZoneUserId());
							map.put("toUserId", userid);
							map.put("authDate", start_time.getText().toString()
									.split(" ")[0].replaceAll("/", "-"));
							// map.put("authFrom",
							// start_time.getText().toString());
				...
				try {
					...
					case 1:
						...
						for (int i = 0; i < chekbleData.size(); i++) {
							for (int j = 0; j < chekbleData.get(i).size(); j++) {
								if (chekbleData.get(i).get(j)) {
									KeyInfo keyInfo = (KeyInfo) myadapter
											.getGroup(i);
									List<Key> keys = keyInfo.getKeys();
									Key key = keys.get(j);
									Intent chatIntent = new Intent();
									chatIntent.putExtra("zoneName",
											keyInfo.getAddress());
									chatIntent.putExtra("zoneType",
											getString(R.string.doorType2));
	...
	public void onSuccess(JSONObject response) {
		// TODO Auto-generated method stub
		FamilyAddr familyAddr = GsonUtli.jsonToObject(response.toString(),
				FamilyAddr.class);
		if (familyAddr != null) {
	...
	class Myadapter extends BaseExpandableListAdapter {
		...
		public View getGroupView(int groupPosition, boolean isExpanded,
			...
			convertView = LayoutInflater.from(AuthKeyActivity.this).inflate(
							...
							for (int k = 0; k < addrAata.size(); k++) {
								if (l1ZoneId.equals(addrAata.get(k))) {
									for (int i = 0; i < isChekble.size(); i++) {
										for (int j = 0; j < isChekble.get(i)
												.size(); j++) {
											isChekble.get(i).set(j, false);
										}
									}
									isChekble.get(groupPosition).set(
											childPosition, true);
