...
import com.icloudoor.cloudoor.chat.DownloadImageTask;
...
public class AlertDialog extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if(path != null){
		    ...
		    if(ImageCacheBitmap.getInstance().get(path) != null){
		        imageView.setImageBitmap(ImageCacheBitmap.getInstance().get(path));
		    }else{
		        Bitmap bm = ImageUtils.decodeScaleImage(path, 150, 150);
		        imageView.setImageBitmap(bm);
		        ImageCacheBitmap.getInstance().put(path, bm);
		    }
