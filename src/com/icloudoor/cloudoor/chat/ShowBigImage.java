...
public class ShowBigImage extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		default_res = getIntent().getIntExtra("default_image", R.drawable.icon_boy_110);
		...
		if (uri != null && new File(uri.getPath()).exists()) {
			...
			bitmap = ImageCacheBitmap.getInstance().get(uri.getPath());
			if (bitmap == null) {
				...
				runOnUiThread(new Runnable() {
					...
					public void run() {
						...
						if (bitmap == null) {
							...
							image.setImageBitmap(bitmap);
							ImageCacheBitmap.getInstance().put(localFilePath, bitmap);
							isDownloaded = true;
						}
						if (pd != null) {
							pd.dismiss();
